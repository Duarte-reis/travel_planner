...
function FormComplete() {
    ...
    /* ---- Calculate NET price per paying passenger ---- */
    ...
    const finalNetPerPayingPaxArray = Object.values(numOfPaxData[0]).map((tier, idx) => {
        ...
        // Sum all costs to get base NET per paying passenger
        let net =
            hotelPriceTotal +
        ...
        // Add single supplement cost for free passengers (if single room is selected)
        if (activeIndex === 0 && free > 0) {
            const singleSupplementForFree = singleSupplementTotal * free;
            const costPerPayingPax = singleSupplementForFree / numOfPax;
            net = net + costPerPayingPax;
        }
        
        // Distribute free passenger costs among paying passengers
        const totalCostOfFreePax = net * free;
        const freeCostPerPayingPax = totalCostOfFreePax / numOfPax;
        net = net + freeCostPerPayingPax;
        
        // Add commission to NET
        const commissionAmount = net * (comissionMargin / 100);
        const commissionRounded = Math.round(commissionAmount);
        net = net + commissionRounded;
        
        return net;
    });

    // Calculate commission in euros for display purposes
    const comissionPerTier = finalNetPerPayingPaxArray.map(netValue =>
        Math.round((netValue / (1 + comissionMargin / 100)) * (comissionMargin / 100))
    );

    // Calculate company profit margin
    const marginPerTier = finalNetPerPayingPaxArray.map(netValue =>
        Math.round(netValue * (profitMargin / 100))
    );

    // Calculate final RRP (Recommended Retail Price) = NET + Company Margin
    const finalRrpPerTier = finalNetPerPayingPaxArray.map((netValue, idx) => 
        netValue + marginPerTier[idx]
    );
